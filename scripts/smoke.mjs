...
await action({action:'simulate-stock',slug:probe.slug,stock:7});
const sync=await action({action:'sync'});
current=await state();
check(sync.updated>=1 && current.products.find(p=>p.slug===probe.slug).stock===7,'Cambio proveedor → tienda: stock 7');
