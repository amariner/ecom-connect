...
`GET /api/demo/sync-control` devuelve configuración, revisión, última señal del ejecutor, copias de catálogo por canal y resultados de conciliación. `POST` acepta `action: "save"` con `revision` y `policy`; `action: "supplier"` con `limit` y `all`; o `action: "marketplaces"`. [Ver campos y comportamiento](SINCRONIZACION.md). Requiere las dos banderas demo y mismo origen para escribir.
