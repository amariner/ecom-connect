...
import { DemoError, cancelOrder, hashText } from './demo';
...
export async function readAccountOrder(db: D1Database, session: AccountSession, publicRef: string) {
  ...
  if (!parsed.success) return null;
  const order = await createD1CustomerAccount(db).readOrder(session.id,parsed.data);
  return order && { ...order, actions:decideAccountOrderActions(order) };
}
