...
import { cancelAccountOrder, cancelAccountReturn, requestAccountReturn, requireAccountSession } from '../../../lib/account';
...
export const POST: APIRoute = (context) => demoApi(context,async () => {
  ...
  if (body?.action === 'devolver') return requestAccountReturn(db,session,body);
  if (body?.action === 'anular-devolucion') return cancelAccountReturn(db,session,body);
