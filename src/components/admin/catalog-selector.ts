...
export function selectorShell(scope:string,expanded=false) {
 return `<details class="catalog-selector" data-selector="${scope}" ${expanded?'open':''}><summary><span><small>${scope==='supplier'?'IMPORTACIÓN A LA TIENDA':'SELECCIÓN INDEPENDIENTE'}</small><strong>${names[scope]}</strong></span><span class="selector-summary">Seleccionar productos <span aria-hidden="true">＋</span></span></summary><div class="selector-content" aria-live="polite">Cargando catálogo…</div></details>`;
}
export function feedSelectors() {
 return `<section class="admin-card spaced-card"><div class="card-heading"><div><span class="section-kicker">TÚ DECIDES QUÉ COMPARTIR</span><h2>Tu feed de productos</h2></div><span class="status-badge success">3 destinos</span></div><div class="card-body"><p class="muted">Tres feeds con selecciones independientes. Guarda los cambios para actualizar su contenido. Solo se exportan productos visibles en la tienda. Las conexiones son de demostración.</p>${['lighthouse','google','meta'].map(scope=>`<div class="specific-feed"><div class="specific-feed-header"><strong>${names[scope]}</strong><a class="text-link" href="/feeds/${scope}.xml" target="_blank" rel="noopener">Abrir XML ↗</a></div><label class="feed-address">URL del feed<input readonly aria-label="URL del feed ${names[scope]}" value="${esc(location.origin)}/feeds/${scope}.xml" /></label>${selectorShell(scope,scope==='lighthouse')}</div>`).join('')}</div></section>`;
}
export function marketplaceSelectors() {
 return `<section class="admin-card spaced-card"><div class="card-heading"><div><span class="section-kicker">SURTIDO POR CANAL</span><h2>Un catálogo para cada marketplace</h2></div></div><div class="card-body"><p class="muted">Cada canal conserva su propia selección. Para publicarse, un producto también debe estar visible en la tienda e incluido en Lighthouse. Guardar actualiza la distribución simulada.</p>${['AMAZON','MIRAVIA','CARREFOUR','EBAY'].map(scope=>selectorShell(scope)).join('')}</div></section>`;
}
