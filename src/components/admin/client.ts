...
type Cancellation = { source: 'panel' | 'marketplace'; reason: string; supplier_outcome: 'pending' | 'not_required' | 'accepted' | 'rejected'; requested_at: string; cancelled_at: string | null; marketplace_synced_at: string | null };
...
function orderCancellation(data: Detail) {
  ...
  if (order.status === 'cancelled') {
    body = cancellation ? `<span class="status-badge danger"><span></span>Pedido cancelado</span><dl class="detail-facts"><div><dt>Origen</dt><dd>${cancellation.source === 'marketplace' ? `Solicitud de ${html(channelName)} demo` : 'Cancelado desde el panel'}</dd></div><div><dt>Motivo</dt><dd>${html(cancellationReasons[cancellation.reason] || cancellation.reason)}</dd></div><div><dt>Respuesta del proveedor</dt><dd>${html(supplierOutcomes[cancellation.supplier_outcome] || cancellation.supplier_outcome)}</dd></div><div><dt>Fecha</dt><dd>${cancellation.cancelled_at ? date(cancellation.cancelled_at) : 'Registro pendiente de completar'}</dd></div>${marketplace ? `<div><dt>Comunicada a ${html(channelName)} demo</dt><dd>${cancellation.marketplace_synced_at ? date(cancellation.marketplace_synced_at) : 'Pendiente de conciliar'}</dd></div>` : ''}</dl>${cancellation.supplier_outcome === 'rejected' ? '<div class="info-box">El proveedor demo expidió unidades mientras se cancelaba. Revisa las expediciones y gestiona su devolución.</div>' : ''}${!marketplace && !cancellation.cancelled_at ? button('Conciliar sincronización', 'sync', true, '', 'refresh') : ''}<p class="form-help">El stock de la tienda se repuso al cancelar. No se realizan reembolsos reales.</p>`
