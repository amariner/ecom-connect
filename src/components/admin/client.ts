...
function orders() {
  ...
  return `${heading('VENTAS CENTRALIZADAS', 'Cada pedido. Un mismo lugar.', 'Gestiona los pedidos de tu tienda y tus marketplaces, de principio a fin.', `<a class="button button-primary" href="/admin/marketplaces">${icon('orders')}Simular pedido</a>`)}
    ...
    ${supplierSyncControls(state.sync_configuration)}${dispatchRunsCard()}<section class="admin-card"><div class="card-heading"><h2>Historial de pedidos</h2><span class="section-kicker">TODOS LOS PEDIDOS DE LA DEMO</span></div><p class="table-help">25 por página · Más recientes primero. Busca por pedido, cliente demo, referencia del proveedor o seguimiento.</p>
...
function supplier() {
  ...
  return `${heading('INTEGRACIONES / PROVEEDOR', 'El origen de tu catálogo.', 'Productos, precios y disponibilidad sincronizados desde un único proveedor.', button('Sincronizar ahora', 'sync'))}
    ...
    ${selectorShell('supplier',true)}<div class="two-panel-grid supplier-panels"><section class="admin-card"><div class="card-heading"><div><span class="section-kicker">PRUEBA EL FLUJO</span><h2>Simula cambios en el proveedor</h2></div>${icon('refresh')}</div>
...
function destinationChannels() {
 return `<section class="admin-card channel-destinations-top"><div class="card-heading"><div><span class="section-kicker">TUS CANALES CONECTADOS</span><h2>Canales de destino</h2></div><a class="text-link" href="/admin/marketplaces">Seleccionar productos por canal ↗</a></div><div class="channel-destination-grid">${state.marketplaces.map(m=>`<div>${channelBadge(m.channel)}<strong>${number(m.orders_count)} <small>pedidos recibidos</small></strong><p>${number(m.published)} productos publicados · ${number(m.pending_supplier)} pedidos pendientes</p><small data-stock-channel="${html(m.channel)}">Stock: ${date(m.stock_synced??undefined)}</small><small>Estados y seguimiento: ${date(m.last_order_sync??undefined)}</small><a class="text-link" href="/admin/pedidos?channel=${m.channel}">Ver pedidos ↗</a></div>`).join('')}</div></section>`;
}
function lighthouse() {
 return `${heading('INTEGRACIONES / CANALES DE VENTA','Marketplaces','Controla tus canales, pedidos y actualizaciones desde un mismo panel.', '<a class="button button-primary" href="#hub-sync">Configurar sincronización ↓</a>')}${destinationChannels()}${channelOverview(state.analytics,true)}${marketplaceSyncControls(state.sync_configuration)}${feedSelectors()}`;
}
...
function dispatchRunsCard() {
  ...
  return `<section id="dispatch-runs" class="admin-card spaced-card" aria-labelledby="dispatch-runs-title"><div class="card-heading"><div><span class="section-kicker">SUPERVISIÓN</span><h2 id="dispatch-runs-title">Ejecuciones del envío agrupado</h2></div><span class="status-badge ${paused ? 'warning' : 'neutral'}"><span></span>${paused ? 'Programado en pausa' : 'Ver estado del ejecutor arriba'}</span></div>${rows ? `<div class="table-scroll" role="region" aria-label="Últimas ejecuciones del envío agrupado" tabindex="0"><table class="shipment-table"><thead><tr><th scope="col">Inicio</th><th scope="col">Resultado</th><th scope="col" class="align-right">Enviados</th><th scope="col" class="align-right">Errores</th><th scope="col" class="align-right">Quedan</th></tr></thead><tbody>${rows}</tbody></table></div>` : ''}<div class="card-body">${rows ? '' : '<p class="muted">Todavía no se ha ejecutado ningún envío agrupado.</p>'}<p class="form-help">Se muestran las cinco últimas. Solo puede haber una ejecución en curso: otra simultánea se anota como omitida. La pausa detiene las ejecuciones programadas, nunca el envío manual. El estado del ejecutor se muestra en la configuración de sincronización.</p></div></section>`;
}
...
async function render() {
  ...
  if (view === 'dashboard') connectNetwork(async signal => {
    ...
    for(const selector of ['.metrics-grid','#channel-overview','#attention','.recent-orders','.activity-card']){
