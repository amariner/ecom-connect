...
function dashboard() {
  ...
  return `${heading('TU CENTRO DE OPERACIONES', 'Todo conectado. Todo bajo control.', 'Tu catálogo, tus pedidos y todos tus canales. En un mismo lugar.', `<a class="button button-secondary" href="/admin/documentacion/guia-demo">${icon('orders')}Guía de la demo</a>${button('Sincronizar ahora', 'sync')}`)}
    ...
    <div class="metrics-grid">${metric('Productos en catálogo', number(state.products.length), `<span class="text-green">${number(active)} activos</span> en tu tienda`, 'box')}${metric('Pedidos recientes', number(state.orders.length), 'Últimos 100 · Web y marketplaces', 'orders')}${metric('Canales de venta', number(state.marketplaces.length + 1), '<span class="text-green">●</span> Tienda web + marketplaces', 'nodes')}${metric('Pendientes de proveedor', number(pending), pending ? 'Entre los pedidos recientes' : 'Sin pendientes en los pedidos recientes', 'clock', pending ? 'metric-tinted' : '')}</div>
    ...
    <div class="dashboard-columns"><section class="admin-card recent-orders"><div class="card-heading"><h2>Últimos pedidos <span class="count-chip">${state.orders.length}</span></h2><a class="text-link" href="/admin/pedidos">Ver todos ↗</a></div>${ordersTable(state.orders.slice(0, 5), true)}</section><section class="admin-card activity-card"><div class="card-heading"><h2>Actividad reciente</h2><span class="live-indicator"><span></span>Actividad demo</span></div>${eventsList(state.events, 4)}<div class="activity-footnote">Cada acción queda registrada en tu demo.</div></section></div>
...
function renderOrders() {
  ...
  document.querySelector('#order-result-count')!.textContent = `${filtered.length} de ${state.orders.length} pedidos`;
...
function marketplaceSyncCard(data: Detail) {
  ...
  const current = sync && sync.supplier_status === data.order.supplier_status && (sync.tracking_number ?? '') === (data.order.tracking_number ?? '');
  return `<section class="admin-card spaced-card"><div class="card-heading"><h2>Retorno al marketplace</h2>${icon('nodes')}</div><div class="card-body"><span class="status-badge ${current ? 'success' : 'warning'}">${current ? 'Acuse demo actualizado' : 'Pendiente de conciliar'}</span><dl class="detail-facts"><div><dt>Canal</dt><dd>${html(channelInfo(data.order.channel).name)}</dd></div><div><dt>Estado comunicado</dt><dd>${sync ? html(label(sync.supplier_status)) : 'Sin notificar'}</dd></div><div><dt>Tracking comunicado</dt><dd>${html(sync?.tracking_number || 'Aún no disponible')}</dd></div><div><dt>Último acuse local</dt><dd>${date(sync?.synced_at)}</dd></div></dl>${!current ? button('Conciliar sincronización', 'sync', true, '', 'refresh') : ''}<p class="form-help">Proveedor → Ecom Connect → Lighthouse → ${html(channelInfo(data.order.channel).name)}. Confirmación guardada en la demo.</p><a class="text-link" href="/admin/documentacion/lighthouse">Contrato de la integración ↗</a></div></section>`;
}
