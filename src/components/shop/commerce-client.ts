...
export async function requestQuote(lines:CartLine[],postal_code:string):Promise<CartQuote>{
 ...
 const data=await response.json().catch(()=>{throw new Error('No hemos podido consultar la cesta. Vuelve a intentarlo en unos instantes.');}) as CartQuote & {error?:string};
